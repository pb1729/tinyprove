...
def typecheck_args(args:List[Tuple[str, IrNode]], ctx:List[Tuple[str, Term]], defns):
  ...
  arg_ty = arg_ty_ir.to_term(ctx_to_names(ctx))
  ...
  return typecheck_args(args_rest, [(arg_nm, arg_ty)] + ctx, defns)
...
class IrInductiveSelfRef(IrNode):
  head: InductiveDefHead
  index_vals: List[IrNode]
  def to_term(self, ctx_nm):
    ans_ir = app_wrap(
      self.head.selfref_ir,
      self.index_vals)
    return ans_ir.to_term(ctx_nm)
...
def walk(head: InductiveDefHead, node:IrNode, polarity:bool=True) -> bool:
  ...
  match node:
    ...
    case IrConst(name):
      ...
      return True
    case IrInductiveSelfRef(selfref_head, index_vals):
      assert head is selfref_head, "Found IrInductiveSelfRef pointing to a different inductive definition!"
      return polarity
    case _:
...
class ConstructorDef:
  def __init__(self, head:InductiveDefHead, name:str, args:List[Tuple[str, IrNode]], output_indices:List[IrNode]):
    ...
    params_ctx = typecheck_args(self.head.params, [], self.head.defns)
    inds_ctx = typecheck_args(self.head.indices, params_ctx, self.head.defns)
    constructor_ctx = typecheck_args(self.args, params_ctx, self.head.defns_ext)
    self.check_positive()
    self.check_output_indices(constructor_ctx, inds_ctx)
  def get_ty(self):
    return pi_wrap(
      pi_wrap(
        IrInductiveSelfRef(self.head, self.output_indices),
        self.args[::-1]),
      self.head.params[::-1])
  ...
  def check_output_indices(self, constructor_ctx:List[Tuple[str, Term]], inds_ctx:List[Tuple[str, Term]]):
    ...
    for i in range(num_inds):
      ...
      if not conv(output_index_ty, target_ty):
...
class InductiveDef:
  def __init__(self, head:InductiveDefHead, constructors:List[ConstructorDef]):
    ...
    self.constructor_tys = {
      constructor.name: constructor.get_ty().to_term([])
  ...
  def get_motive_ty(self, output_ty:IrNode) -> IrNode:
    ...
    return pi_wrap(
      IrPi("@instance",
        IrInductiveSelfRef(self.head, self.get_index_vars()),
        output_ty),
      self.head.indices[::-1])
    ...
    match key:
      case ["ind", sortnum]:
        ...
        if sortnum not in self.ind_tys:
          ...
          self.ind_tys[sortnum] = ind_ty_ir.to_term([])
